...
function resolveRecordActivityCharacter(value: unknown): string | null {
  ...
  return activityText ? Array.from(activityText).at(-1) ?? null : null;
}
...
export function resolveCodexRolloutActivityTailCharacter(rawTail: string): string | null {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    try {
      ...
      // A JSONL record is authoritative only after the writer finishes its newline.
    }
  }
  return null;
}

export async function readCodexRolloutActivityTailCharacter(filePath: string, tailBytes = DEFAULT_TAIL_BYTES): Promise<string | null> {
