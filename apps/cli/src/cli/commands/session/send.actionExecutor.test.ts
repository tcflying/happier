...
describe('happier session send (action executor)', () => {
  it('routes through ActionExecutor with the expected action id and args', async () => {
    ...
    try {
      await handleSessionCommand(['send', 'sess-1', 'Hello', '--local-id', 'fusion-msg-1', '--permission-mode', 'read_only', '--model', 'gpt-4o', '--wait', '--timeout', '30', '--json'], {
  ...
  it('rejects an invalid explicit local id before authentication or dispatch', async () => {
    execute.mockClear();
    const { handleSessionCommand } = await import('./handleSessionCommand');

    await expect(handleSessionCommand(['send', 'sess-1', 'Hello', '--local-id', 'contains spaces'], {
