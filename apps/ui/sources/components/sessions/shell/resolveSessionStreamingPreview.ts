...
function finalCharacter(value: unknown, depth = 0): string | null {
    ...
    if (typeof value === 'number' || typeof value === 'boolean' || typeof value === 'bigint') return Array.from(String(value)).at(-1) ?? null;
...
function finalAgentActivityCharacter(message: Message): string | null {
    ...
    return finalCharacter([message.tool.name, message.tool.input, message.tool.description, message.tool.state, message.tool.permission, message.tool.result]);
}
