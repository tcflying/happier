...
export const ja: TranslationStructure = {
  ...
  status: {
    ...
    working: "作業中...",
    thinking: "思考中…",
    thinkingActivities: "思考中|処理中|計算中|解析中|検討中|構築中",
...
settingsSession: {
  ...
  chatFooter: {
    ...
    directSessionControlledByCurrentHappier: ({ session, pid }: { session: string; pid: string | number }) => `この Happier セッションが占有中です（セッション ${session}、プロセス ${pid}）。`,
    directSessionControlledByOtherHappier: ({ session, pid }: { session: string; pid: string | number }) => `別の Happier セッションが操作中です（セッション ${session}、プロセス ${pid}）。`,
